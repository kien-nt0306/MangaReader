...
const DragAbleSortList = ({ navigation }) => {
  ...
  let tiles = JSON.parse(storage.getString(CONSTS.AUTHOR_LIKE_KEY) || '[]')
  useEffect(() => {
    tiles = JSON.parse(storage.getString(CONSTS.AUTHOR_LIKE_KEY) || '[]')
    return (() => {
      storage.set(CONSTS.AUTHOR_LIKE_KEY, tiles)
    })
